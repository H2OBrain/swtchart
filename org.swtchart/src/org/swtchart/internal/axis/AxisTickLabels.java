...
public class AxisTickLabels implements PaintListener {
	...
	private void updateTickLabelForLogScale(int length) {
		final int max_labels = (int)Math.floor((double)length / axis.getTick().getTickMarkStepHint());
		...
		double off   = min;
		int	poff  = (int)Math.floor(Math.log10(off));
		double dofff = off/Math.pow(10, poff);
		//rng
		double rng   = max-min;
		int	prng  = (int)Math.floor(Math.log10(rng));
		double drngf = rng/Math.pow(10, prng);
		//
		int pmin, pmax;
		double dminf, dmaxf;
		if (poff < prng) {
//			System.out.println("poff <  prng");
			dminf = dofff;
//			if (Math.ceil(dminf)==0) { System.out.println("fail!!!!!"); }
			pmin  = poff;
			dmaxf = max/Math.pow(10, prng);
			pmax  = prng;
			off   = 0;
		} else {
//			System.out.println("poff >= prng");
			...
			dmaxf = drngf;
			pmax  = prng;
			off   = dofff*Math.pow(10, poff);
		}
//		int dmin,dmax;
//		dmin = (int)Math.round(dminf); //Math.ceil
//		dmax = (int)Math.round(dmaxf); //Math.floor
//		System.out.printf("off:%.3e poff:%d prng:%d doff:%.3e drngf:%.3e\n", off,poff,prng,dofff,drngf);
//		System.out.printf("min: %de%d (%.12e)\n", dmin,pmin,dminf*Math.pow(10, pmin+off));
//		System.out.printf("max: %de%d (%.12e)\n", dmax,pmax,dmaxf*Math.pow(10, pmax+off));
//		double min_err = ((dminf*Math.pow(10, pmin+off)) - min)/min;
//		double max_err = ((dmaxf*Math.pow(10, pmax+off)) - max)/max;
//		if (min_err>1e-6) {
//			System.out.println("min error too big!");
//		}
//		if (max_err>1e-6) {
//			System.out.println("max error too big!");
//		}
		double num_steps;
		if (pmin==pmax) {
//			System.out.println("pmin==pmax");
			num_steps = dmaxf-dminf;
		} else {
			num_steps = 10-dminf + 10*(pmax-pmin-1) + dmaxf;
		}
		
		double step=1; int step_state=0; int dir=0;
		while (true) {
//			System.out.printf("num_steps: %.3f (%d), step: %.3e\n",num_steps, (int)Math.floor(num_steps)+1,step);
			if ((dir!=-1) && (num_steps>max_labels)) {
				switch (step_state) {
					case 0 :
						num_steps/=5; step*=5; step_state=1; dir=1;
						continue;
					case 1 :
						num_steps/=2; step*=2; step_state=0; dir=1;
						continue;
					default :
						System.err.println("Invalid step_state!");
						break;
				}
			}
			if ((dir!=1) && (num_steps<max_labels)) {
				switch (step_state) {
					case 0 :
						num_steps*=2; step/=2; step_state=1; dir=-1;
						continue;
					case 1 :
						num_steps*=5; step/=5; step_state=0; dir=-1;
						continue;
					default :
						System.err.println("Invalid step_state!");
						break;
		...
		int pstep;
//		double dstepf;
		pstep  = (int)Math.floor(Math.log10(step));
//		dstepf = step/Math.pow(10, pstep);
//		int dstep  = (int)Math.floor(dstepf);
		int istep  = (int)Math.round(step);
		
		int steps[];
		if (step_state == 0) { // 1-step
			steps = new int[] {1,2,3,4,5,6,7,8,9};
		} else {
//		if (step_state == 1) { // 5-step
			steps = new int[] {1,3};
		}
		pmin += pstep;
		pmax += pstep;
		...
		//	pmax += 1;
		//}
		//dmin  = (int)Math.ceil(dminf/Math.pow(10, pstep));
		//dmax  = (int)Math.floor(dmaxf/Math.pow(10, pstep));
		while (dminf>=10) {
//			dmin  /= 10;
			dminf /= 10;
			pmin  += 1;
...
//		System.out.printf("dstep: %d\n", dstep);
//		System.out.printf("pstep: %d\n", pstep);
//		System.out.printf("min: %de%d (%.12e)\n", dmin,pmin,dminf*Math.pow(10, pmin+off));
//		System.out.printf("max: %de%d (%.12e)\n", dmax,pmax,dmaxf*Math.pow(10, pmax+off));
//		min_err = ((dminf*Math.pow(10, pmin+off)) - min)/min;
//		max_err = ((dmaxf*Math.pow(10, pmax+off)) - max)/max;
//		if (min_err>1e-6) {
//			System.out.println("min error too big!");
//		}
//		if (max_err>1e-6) {
...
//		int i=0;
		int p = pmin;
		while (true) {
			off = Math.ceil(off/Math.pow(10, p))*Math.pow(10, p);
//			System.out.printf("p: %d,  off: %.12eprintf", p,off);
			for (int d : steps) {
				double v = off+d*Math.pow(10, p);
				if (v < min) { continue; }
				if (v > max) {
//					System.out.println("done. (A)\n");
					return;
				}
				if (tickLabels.size()>100) {
					System.err.println("Too many tick labels!");
					return;
				}
//				System.out.printf("%3d : %.13e\n", i,v);
				
				if (axis.isDateEnabled()) {
					Date date = new Date((long) v);
					tickLabels.add(format(date));
				} else {
					tickLabels.add(format(v));
				}
				tickLabelValues.add(v);

				int tickLabelPosition =
						(int)(
							(Math.log10(v)   - Math.log10(min))
...
//			if (p==pmax) { System.out.println("not finished?"); }
			p+=1;
		}
//		
//		
//		final int gridStepCountHint = (int)Math.floor((double)length / axis.getTick().getTickMarkStepHint());
//
//		double min = axis.getRange().lower;
//		double max = axis.getRange().upper;
//		// simple guess..
//		int num_labels = (int)(Math.log10(max/min)*10);
//		if (num_labels==0) {
//			num_labels = (int)Math.ceil(Math.pow(10, 1+Math.log10(max)-Math.log10(min)));
//		}
//		
//		int digitMin  = (int) Math.floor(Math.log10(min));
//		int digitMax  = (int) Math.ceil (Math.log10(max));
//		int digitDiff = (int) Math.floor(Math.log10(max-min));
//		
...
////		BigDecimal tickStepNext = pow(10, digitMin);
//		BigDecimal tickStep	 = pow(10, digitDiff - 1);
//		BigDecimal tickStepNext = pow(10, digitDiff);
...
//			BigDecimal minDiff = MIN.remainder(tickStepNext).divide(tickStepNext);
//			if (minDiff.compareTo(BigDecimal.valueOf(0.5)) <= 0) {
//				cnt = 5-1;
//			} else {
//				cnt = 0;
//			}
//		}
//		
//		boolean sub_steps = true, single_sub_step = false;
//		if (num_labels > gridStepCountHint) {
//			if (num_labels/5 > gridStepCountHint) {
//				sub_steps = false;
//			} else {
//				single_sub_step = true;
//				if (cnt == 0) {
//					firstPosition = tickStepNext;
//				} else {
//					firstPosition = tickStepNext.divide(BigDecimal.valueOf(2));
//				}
//			}
//		} else
//		if (num_labels <= gridStepCountHint/10) {
//			tickStepNext = tickStep;
//			tickStep = tickStep.divide(BigDecimal.valueOf(10));
//		} else
//		if (num_labels <= gridStepCountHint/2) {
//			tickStepNext = tickStep;
//			tickStep = tickStep.divide(BigDecimal.valueOf(2));
//		}
//		
//		for (int i = digitMin; i <= digitMax; i++) {
//			for (BigDecimal j = firstPosition; j.compareTo(pow(10,i))<=0; j = j.add(tickStep)) {
//				if (j.compareTo(MAX)>0) {
//					break;
//				}
//				
//				cnt++;
//				if (single_sub_step && ((cnt!=1)&&(cnt!=5))) {
//					tickLabels.add("");
//				} else {
//					if (axis.isDateEnabled()) {
...
//				if (!sub_steps) break;
//			}
//			firstPosition = tickStep = tickStep.multiply(BigDecimal.valueOf(10));
...
//		}
	...
	private void updateTickVisibility() {
		...
		for (int i = 0; i < tickLabelPositions.size(); i++) {
			...
			if (!axis.isValidCategoryAxis()) {
				/* disabled because labels/ticks are now created in a "better" way.. */
